...
function foodbank_achievement_award($content, $user_id, $activity_id)
{
    ...
    if (empty($achievement_id) || $achievement_id !== "9965") {
        return;
    }

    if (!isset($_POST['mpp-attached-media'])) {
        echo '<script>alert("請附上物資送達之照片");</script>';
        $args = array(
            'id'                => $activity_id,
        );
    ...
    foreach ($attached_media_array as $index => $media) {
        ...
        GLOBAL $wpdb;
                    ...
                    $query = "SELECT * FROM $table_name where user_id = $user_id and post_id = 9965 ORDER BY user_earning_id DESC LIMIT 1";
    ...
    $args  = array(
        ...
        'gooddeed_token'               => 1,
        'gooddeed_token_granted_once'  => 600,
        'completed_numbers'            => 1,
        'carbon_token_granted_total'   => 0,
        'gooddeed_token_granted_total' => 600,
    ...
    gamipress_award_points_to_user( $user_id, 600, 'gooddeed-token' );
    ...
    update_option( 'tree_love_womens_day_total_gooddeed_token', $tree_love_womens_day_total_gooddeed_token );
                ...
                $arg=array(
                  ...
                  // Optional: The content of the activity item e.g. "BuddyPress is awesome guys!".
                    ...
                    'item_id'           => 13,                  // Optional: The ID of the specific item being recorded, e.g. a blog_id.
                    'recorded_time'     =>  $time, // The GMT time that this activity was recorded.
                    'error_type'        => 'bool',
                );
                bp_activity_add($arg);

                $success_message='謝謝您的幫助，已將600點好人幣，發送給您';
    ...
    add_action('bp_activity_posted_update', 'foodbank_achievement_award', 9999, 3);
