					...
					<div class="socialv-member-main">
						...
						<div class="socialv-member-center item-block">
							<div class="member-name">
								...
								$user_display_name= $members_template->member->display_name;
								if (substr($user_display_name, 0, 2) === "09" && strlen($user_display_name) === 10) {
									// 取得原始字串除了末三碼的部分
									$prefix = substr($user_display_name, 0, 7);
									
									// 新字串
									$newString = $prefix . "xxx";
									
									// 輸出新字串
									//$user_display_name= $newString;
								} 
									echo '<a href="' .bp_core_get_user_domain( $members_template->member->user_id, $members_template->member->user_nicename, $members_template->member->user_login ) . '">' . $user_display_name . '</a>'
						...
						<?php echo gamipress_get_points_type_thumbnail('gooddeed-token').'好人幣:'. gamipress_get_user_points(bp_get_group_member_id(),'gooddeed-token');?>
							<br />
							<?php echo gamipress_get_points_type_thumbnail('carbon-token').'碳幣:'. gamipress_get_user_points(bp_get_group_member_id(),'carbon-token');?>
							<br />
						</div>
					</div>
