...
<div class="row card-space">
		...
		<div class="card-main card-space">
			...
			<div class="card-inner">
				<div class="row">
					...
					<form action="<?php echo bp_displayed_user_domain() . bp_get_profile_slug() . '/change-avatar'; ?>" method="post" class="standard-form1" id="settings-form">

						您可以選擇我們提供的大頭貼<br />
					<input type="text" name="summ-avatar-image" id="summ-avatar-image"  maxlength="6" class="form-control" size="16" value="" class="settings-input small password-entry" <?php bp_form_field_attributes('dfff'); ?> placeholder="<?php esc_attr_e('請輸入您收到的驗證碼', 'socialv'); ?>" />							
							<div class="form-edit-btn">
							<div class="submit">
								<input type="submit" name="summ-avatar-image-submit" value="<?php esc_attr_e('Save Changes', 'socialv'); ?>" id="submit" class="auto btn socialv-btn-success" />
							</div>
						</div>
						<?php wp_nonce_field('bp_summ_user_avatar_image'); ?>
	...
	$pictures_ids = mpp_activity_get_attached_media_ids(4616);
	//error_log( print_r($pictures_ids, true) );
